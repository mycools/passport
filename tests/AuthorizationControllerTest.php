...
class AuthorizationControllerTest extends TestCase
{
    ...
    {
        Mycools\Passport\Passport::tokensCan([
            'scope-1' => 'description',
        ]);
        ...
        $controller = new Mycools\Passport\Http\Controllers\AuthorizationController($server, $response);
        ...
        $authRequest->shouldReceive('getScopes')->andReturn([new Mycools\Passport\Bridge\Scope('scope-1')]);
    ...
    public function test_request_is_approved_if_valid_token_exists()
    {
        ...
        $authRequest->shouldReceive('getScopes')->once()->andReturn([new Mycools\Passport\Bridge\Scope('scope-1')]);
        ...
        $tokens = Mockery::mock('Mycools\Passport\TokenRepository');
        $tokens->shouldReceive('findValidToken')->with($user, 'client')->andReturn($token = Mockery::mock('Mycools\Passport\Token'));
